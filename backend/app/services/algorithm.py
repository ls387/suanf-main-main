...
class SchedulingService:
    ...
    def run_scheduling(self, version_id: int, ga_config: Dict) -> Dict:
        ...
        try:
            ...
            coverage_rate = (
                (scheduled_tasks / total_tasks * 100) if total_tasks > 0 else 0
            )
            ...
            # 计算质量指标（新增）
            metrics = self.calculate_metrics(best_solution, ga.task_dict, data)
            ...
            return {
                ...
                "conflicts": conflicts,
                "average_utilization_rate": metrics.get("average_utilization_rate"),
                "classroom_reuse_rate": metrics.get("classroom_reuse_rate"),
            }
    ...
    def calculate_metrics(self, solution, task_dict, data) -> Dict:
        """计算排课质量指标

        Returns:
            包含以下字段的字典：
            - average_utilization_rate: 平均容量利用率(%)
            - classroom_reuse_rate: 教室复用率(0-100)
        """
        from collections import defaultdict
        ...
        try:
            ...
            # 计算教室复用率
            # 复用率 = 一天内教室被排课占用的时段总数 / (教室总数 × 每天最大时段数) × 100%
            daily_classroom_slots = defaultdict(
                int
            )  # (weekday, classroom_id) -> 被占用的时段数

            for gene in solution:
                task = task_dict[gene.task_id]
                for slot in range(gene.start_slot, gene.start_slot + task.slots_count):
                    key = (gene.week_day, gene.classroom_id)
                    daily_classroom_slots[key] += 1

            # 统计每天被使用的教室-时段总数
            total_used_slots = sum(daily_classroom_slots.values())

            # 计算最大容量：教室数 × 平均每天时段数
            # 假设每天有13节课（1-13），周一到周五共5天
            total_classrooms = len(data["classrooms"])
            slots_per_day = 13  # 根据系统配置，通常是1-13节
            days_per_week = 5
            max_possible_slots = total_classrooms * slots_per_day * days_per_week

            classroom_reuse_rate = (
                (total_used_slots / max_possible_slots * 100)
            ...
            return {
                ...
                "classroom_reuse_rate": round(classroom_reuse_rate, 2),
            }
        except Exception as e:
            logger.warning(f"计算指标时出错: {e}")
            return {
                "average_utilization_rate": None,
                "classroom_reuse_rate": None,
            }
